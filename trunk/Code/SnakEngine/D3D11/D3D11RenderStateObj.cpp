...
namespace SE
{
	...
	//	ID3D11DevicePtr const & d3d_device = checked_cast<D3D11Render*>(&MonFac::Only().InstFactory().InstRender())->GetD3DDevice();
	...
	//{
	//	D3D11Render& re = *checked_cast<D3D11Render*>(&MonFac::Only().InstFactory().InstRender());
	...
	//{
	//	D3D11Render const& render_eng = *checked_cast<D3D11Render const *>(&MonFac::Only().InstFactory().InstRender());
