...
public:
	...
	inline void SetColumn(int column, Vector3 &vec, float vec4)
	{
		assert(column>=0 && column<4);
		float *p = &m_00 + column;
		p[0] = vec.GetX(); 
	...
	{
		SEMath::Swap(m_01,m_10);
		SEMath::Swap(m_02,m_20);
		SEMath::Swap(m_03,m_30);
		SEMath::Swap(m_12,m_21);
		SEMath::Swap(m_13,m_31);
		SEMath::Swap(m_23,m_32);
	...
	void Rotate( Vector4CPU &v, float radian)
	{
		...
		float fSin, fCos;
		SEMath::SinCos(radian, fSin, fCos);
